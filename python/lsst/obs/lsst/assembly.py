...
from lsst.obs.base import bboxFromIraf
from lsst.geom import Box2I, Extent2I, SpherePoint, degrees
from lsst.afw.geom import makeSkyWcs
from lsst.ip.isr import AssembleCcdTask
import lsst.afw.cameraGeom as cameraGeom  # noqa F811


def makeWcsFromDetector(camera, detector, boresight, rotation=0*degrees, flipX=False):
    ...
    """
    # TODO: We won't need to pass in a Camera once we can get YamlCameras
    # constructed in the way recommended after DM-14980.
...
def attachRawWcsFromBoresight(exposure, camera):
    ...
    camera : `lsst.afw.cameraGeom.Camera`
        Complete camera geometry information.
    ...
    try:
        ratel, dectel = metadata.getScalar("RATEL"), metadata.getScalar("DECTEL")
        rotangle = metadata.getScalar("ROTANGLE")*degrees
    except KeyError:
    ...
    if ratel == '' or dectel == '' or rotangle == '':  # FITS for None
        ...
        boresight = SpherePoint(ratel, dectel, degrees)
        exposure.setWcs(makeWcsFromDetector(camera, exposure.getDetector(), boresight,
                                            90*degrees - rotangle))
        return True
...
def fixAmpGeometry(amp, bbox, metadata, logCmd=None):
    ...
    if amp.getRawBBox() != bbox:  # Oh dear. cameraGeom is wrong -- probably overscan
        ...
        w,  h  = bbox.getDimensions()  # noqa E221, E241
